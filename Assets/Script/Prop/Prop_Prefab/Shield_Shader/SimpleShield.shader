Shader "MyShader/SimpleShield" {
	Properties {
		_MainColor ("Color" , Color) = (1,1,1,1)   // 主色
		_HighlightColor("HighlightColor" ,Color) = (0,0,1,1) //高光色
		_EdgePow("Threshold" , Range(0 , 5)) = 0.5      // 相交处高光宽度调整值
		_RimNum("Rim" , Range(0 , 5)) = 1            // 边缘光幅度
		_MainTex("Main Tex" , 2D) = "white"{}        // 主贴图
		_MaskTex("Mask Tex" ,  2D) = "white" {}       // uv 移动遮罩贴图
		_speed("Speed" ,Range(0 , 2)) = 1.0            // uv 移动速度
	}
	...
	Pass{
		...
		{
			// 六边形图案边面 alpha 值调整（1变0，0变1，中间差值）
